...
// Closure adicionada
// --------------------
// Cria um formatador de telefone com prefixo fixo
function criarFormatadorTelefone(prefixo) {
    return function(telefone) {
        const nums = telefone.replace(/\D/g, '');
        if (nums.length === 11) {
            return `${prefixo} (${nums.substring(0,2)}) ${nums.substring(2,7)}-${nums.substring(7)}`;
        }
        return prefixo + " " + telefone;
    }
}

// Exemplo de uso da closure
const formatarTelefoneBrasil = criarFormatadorTelefone("+55");
// console.log(formatarTelefoneBrasil("85912345678")); 
// Saída: +55 (85) 91234-5678
